...
| 方法 | 核心思想 | 公式关键 | 适用场景 |
|------|---------|---------|---------|
| 归因窗口 | 等待标签成熟 | $W = F_d^{-1}(0.95)$ | 简单快速上线 |
| DFM (EM) | 假负样本软标签 | $w_i = \frac{p \cdot S}{1-p+p \cdot S}$ | 主力方法 |
| DEFER | 重要性采样 | $w_{neg} = \frac{1-p}{1-p+p \cdot S}$ | DFM 简化替代 |
| 生存分析 | 时间到事件建模 | $S(t) = e^{-H(t)}$ | 长延迟场景 |
| ES-DFM/ESDF | 全空间+DFM | 三塔联合 | 大厂终极方案 |
...
6. **延迟转化**：DFM EM 推导（w_i 公式）、生存分析 vs 分类模型选择、归因窗口对模型影响
7. **LTV建模**：ZILN损失函数，零膨胀+对数正态两部分
...
4. **LLM广告文案说服力超人类**（来源：`LLM时代广告系统技术演进.md`）：通用心理说服原则维度LLM生成广告59.1%胜率超越人类专家，个性化维度持平；RARE用LLM生成商业意图做语义广告检索(EMNLP'25)
5. **品牌广告轻量MPC出价**（来源：`autobidding-uncertainty-aware.md`）：等保回归+MPC滚动优化，纯在线无ML管线，品牌广告场景几百次竞价即收敛
...
| **rec-sys粗排蒸馏** | `粗排蒸馏模型专题.md` | Logit/排序/特权特征/在线蒸馏6法 + COLD |
...
| **DFM延迟转化** | 假负样本是缺失数据，EM 算法：E 步算 w_i=pS/(1-p+pS) fake neg 概率，M 步用软标签训练 |
| **生存分析CVR** | 转化是"事件发生时间"问题，S(t)=exp(-H(t))，右删失=还没转化≠不会转化 |
