...
**今日面试主题**：广告系统（混排、LTR→RL、偏差治理）

**掌握较好**：
- eCPM 排序公式和混排的动机，能清晰分层讲解
- LambdaMART 原理和 λ 梯度的物理含义
- 位置偏差 + IPS 方法的直觉和局限
- ESMM 全空间建模解决样本选择偏差

**需要加强**：
1. **RL 在混排中的具体实现细节**：知道用 PPO/DQN，但状态空间设计、reward shaping 的工程细节不够扎实。明天补充 RL 在 Feed 场景的具体 case study。
2. **DPP（行列式点过程）的数学推导**：知道 DPP 用于多样性，但核心矩阵 L 的构建和采样算法（k-DPP）的复杂度分析还不熟。
3. **在线拉格朗日乘子更新的稳定性问题**：提到了 PID 控制，但对具体的收敛保证和工业调参经验缺乏。
4. **多智能体 RL（MARL）在广告竞价中的落地案例**：理论上知道方向，缺少具体的工业实践引用。

**明天补充计划**：
- 找 1-2 篇 RL reranking 的工业论文精读（DRL for page-wise recommendations）
- 复习 DPP 数学推导，尝试手写一个简单的 k-DPP 采样
- 搜集 MARL 在广告竞价中的案例（阿里 MAAB 等）
