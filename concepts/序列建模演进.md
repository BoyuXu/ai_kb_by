...
7. **CNN 在序列推荐中还有竞争力吗？** → ConvRec (2605.04723) 证明了层次化 CNN 以 $O(n)$ 复杂度在 4 个数据集上超越 SASRec/BERT4Rec。长序列 + 属性丰富 + 推理效率要求高的场景优选 CNN。详见 [[20260504_semantic_id_and_ondevice_rec]]。
8. **频域能增强序列兴趣建模吗？** → FEDIN (2605.01726) 发现正/负 target 条件下注意力的频谱熵有显著差异（真兴趣→低熵，噪声→高熵），target-aware spectrum filtering 选择性保留兴趣信号，对噪声鲁棒性显著提升。详见 [[20260504_semantic_id_and_ondevice_rec]]。
9. **跨域序列推荐如何建模不规则时间间隔？** → BST-CDSR (2605.02369) 用 Neural ODE 连续时间偏好演化 + 反事实扰动增强时间敏感性 + 自适应域迁移权重。详见 [[20260504_semantic_id_and_ondevice_rec]]。
...
- [[20260513_ads_bidding_creative_benchmark|SUIN: 相似用户行为序列拼接增强稀疏用户序列建模 + User-Specific Position Encoding]]
- [[面试项目讲故事_30个案例|面试项目讲故事_30个案例]]
- [[bytedance_recsys_transformer|字节系搜广推Transformer演进(HyFormer/OneTrans统一架构)]]
- [[20260504_speculative_decoding_longcontext_quant|20260504 不可能三角: Efficiency x Compactness x Recall 三选二，52种架构分类]]
- [[20260513_multibehavior_scaling_multitask|SeqRec Scaling Law(纯ID power-law验证) / ApEn数据质量度量 / MBGen多行为序列生成式建模 / EST统一序列建模]]
