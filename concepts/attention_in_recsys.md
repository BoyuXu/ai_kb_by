...
6. **CADET 的 Self-Gated Attention 是什么？** → CADET (LinkedIn) 在 representation-level 和 interaction-level 引入门控，$\text{SGA}(Q,K,V)$ 在 attention score 和 value 上同时做 gating，解决大规模 Decoder-Only 训练不稳定问题。在线 +11.04% CTR。详见 [[20260420_ads_ctr_foundation_and_evaluation]]。
