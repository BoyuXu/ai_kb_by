...
| 固定长度低效 | 胶囊软路由+置信度驱动变长 | CapsID (2605.05096) |
| Embedding 不稳定 | 语义前缀 N-gram 参数化 | SID Prefix Ngram (Meta, 2504.02137) |
| HBM 资源争抢 | RL 控制器动态 EMB/KV cache 分区 | HELM (2605.04450) |
...
### 11.1 LLM 广告的更多维度（2025-2026 新进展）

生成式范式在广告系统的延展远不止拍卖：

- **RARE (EMNLP 2025)**：LLM 生成 Commercial Intention（商业意图自然语言描述）替代传统 DocID，实现意图级语义广告检索。这是生成式检索在搜索广告中的直接应用——从"生成 item ID"到"生成意图描述"。详见 [[LLM时代广告系统技术演进]]。
- **LLM 广告文案生成**：实验证明 LLM 在通用心理说服原则（权威/稀缺/共识/认知）维度的广告文案已超越人类专家（59.1% vs 40.9%），但个性化维度持平。生成式能力从推荐的"选什么"扩展到广告的"怎么说"。
- **四模块框架（Feizi et al., 2023）**：Modification / Bidding / Prediction / Auction 四模块分别对应生成式范式的不同切入点，为后续 LLM-Auction、RARE、GRAD 等工作提供了统一视角。

---

## 相关 Synthesis

- [[从Transformer到生成式推荐_双线演进路线|Transformer→GR双线演进路线（架构线×特征线对照）]]
...
- [[20260513_ads_bidding_creative_benchmark|TencentGR: 全模态生成式广告推荐基准(协同ID+视觉+文本) / CTR-DPO: LLM广告创意+在线CTR偏好优化]]
- [[20260513_multibehavior_scaling_multitask|MBGen: 多行为生成式推荐(behavior+item token交替自回归) / Position-Routed Sparse Architecture]]
