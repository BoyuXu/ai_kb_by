...
> **竞价与多目标的全链路一致性**：自动出价时代，eCPM = pCTR x pCVR x Bid 贯穿召回→排序→出价全链路。BAR 将 bid 信号引入召回阶段保证多阶段一致性；RobustBid 在 CTR/CVR 噪声下保证出价鲁棒性；MDP 框架将用户长期响应纳入多目标优化。详见 [[20260504_ads_bidding_cvr_mechanism|竞价+CVR+机制设计综合]]。
...
- [[20260513_ads_bidding_creative_benchmark|竞价+创意+CTR基准: UniMVT因果去偏多目标(CTR+Uplift联合优化) / ABA多渠道预算组合Bandit / BiCB约束优化]]
- [[20260513_multibehavior_scaling_multitask|多行为+Scaling+多任务前沿: DML(tower间KL互学习) / RMTL(RL动态多任务权重) / MBGen(多行为生成式统一) / EST(CTR Scaling Law)]]
