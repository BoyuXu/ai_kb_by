...
**记忆法**：Uplift 建模像"药物临床试验"——不是问"吃药后病好了吗"（CTR 预估），而是问"因为吃药才好的比例是多少"（因果增量）。四象限用户就像：必然痊愈（Sure Things）、吃药反而过敏（Sleeping Dogs）、吃药才好（Persuadables=目标用户）、无药可救（Lost Causes）。S-Learner=一个医生看所有人，T-Learner=两个医生分开看。
