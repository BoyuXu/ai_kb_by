...
$$
\text{bid} = \text{CPA}_{	ext{target}} \times \hat{p}(\text{CVR}) \times \hat{p}(\text{CTR})
$$

推导：
- 广告主目标：每次有效转化成本 $\leq$ CPA\_target
- 每次点击费用（CPC）：$\text{CPC} = \text{CPA}_{	ext{target}} \times p(\text{CVR})$
- 每次曝光出价（CPM）：$\text{CPM} = \text{CPC} \times p(\text{CTR}) = \text{CPA}_{	ext{target}} \times p(\text{CVR}) \times p(\text{CTR})$
