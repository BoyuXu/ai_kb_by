...
| 方案 | 之前方案 | 创新 | 适用场景 |
|------|---------|------|--------|
| Platt Scaling | 原始 sigmoid 输出 | **后处理逻辑回归校准** | 通用 |
| 等渗回归 | Platt（假设单调） | **非参数分段校准** | 非单调场景 |
| 温度缩放 | 独立校准 | **单参数全局校准** | 多分类 |
| Field-aware | 全局校准 | **按特征域分桶校准** | 广告 oCPC |

```mermaid
graph LR
    Raw[原始分数] --> Platt[Platt Scaling]
    Raw --> Isotonic[等渗回归]
    Platt --> Temp[温度缩放]
    Temp --> Field[Field-aware 分域校准]
```
...
$$
\text{bid} = \text{CPA}}_{\text{{\text{target}}} \times \hat{p}(\text{CVR}) \times \hat{p}(\text{CTR})
$$

推导：
- 广告主目标：每次有效转化成本 $\leq$ CPA\_target
- 每次点击费用（CPC）：$\text{CPC} = \text{CPA}}_{\text{{\text{target}}} \times p(\text{CVR})$
- 每次曝光出价（CPM）：$\text{CPM} = \text{CPC} \times p(\text{CTR}) = \text{CPA}}_{\text{{\text{target}}} \times p(\text{CVR}) \times p(\text{CTR})$
