...
**记忆法**：RLHF 三阶段像"培养员工"——SFT 是"入职培训"（学基本规范），Reward Model 是"绩效考核标准"（从人类偏好学什么是好回答），PPO 是"在职优化"（按考核标准持续改进）。KL 惩罚是"别为了讨好考核而忘了本职技能"。Reward Hacking 就是"为了 KPI 造假"。

**核心考点**：
1. Bradley-Terry 偏好模型：P(y_w > y_l) = σ(r(x,y_w) - r(x,y_l))，RM 损失 = -log σ(Δr)
2. KL 惩罚的作用？（约束策略不偏离 SFT 太远，防止 Reward Hacking）
3. PPO-Clip 为什么需要裁剪？（限制单步更新幅度防止策略崩溃，是 TRPO 的简化替代）
4. GRPO vs PPO 的核心区别？（GRPO 用组内相对奖励替代价值网络，显存减半）
5. Reward Hacking 的根因和解决方案？（RM 是不完美代理；用 KL 惩罚/RM 集成/可验证奖励解决）
...
def grpo_advantage(rewards):
    """GRPO: 组内归一化奖励作为优势估计（无需 value network）"""
    return (rewards - rewards.mean()) / (rewards.std() + 1e-8)

# 模拟 RM 训练
r_chosen = torch.tensor([2.1, 1.8, 3.0])
r_rejected = torch.tensor([0.5, 1.2, 1.5])
loss = reward_model_loss(r_chosen, r_rejected)
print(f"RM loss: {loss.item():.4f}")

# 模拟 GRPO 优势
rewards = torch.tensor([0.8, 0.2, 1.0, 0.5, 0.3])
advantages = grpo_advantage(rewards)
print(f"GRPO advantages: {advantages.round(decimals=2)}")
```

**常见踩坑**：
1. 混淆 RM 训练数据格式——输入是 (prompt, chosen, rejected) 三元组，不是单条打分
2. 忽略 KL 惩罚系数 β 的重要性——β 太小导致 Reward Hacking，β 太大导致模型不学习新能力
3. 认为 GRPO 完全替代 PPO——GRPO 适合有明确答案的场景（数学/代码），开放式生成 PPO 仍更优
