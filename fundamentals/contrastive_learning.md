...
| 方案 | 之前方案 | 创新 | 适用领域 |
|------|---------|------|---------|
| SimCLR | 有监督预训练 | **无标注数据增强 + 对比** | CV |
| MoCo | SimCLR（需大 batch） | **动量编码器 + 负样本队列** | CV |
| InfoNCE | Triplet Loss | **多负样本对比 + 温度控制** | 通用 |
| 双塔召回 | 随机负采样 | **Batch 内负样本 + 纠偏** | 推荐/搜索 |

```mermaid
graph LR
    CL[对比学习核心<br/>正样本拉近 负样本推远]
    CL --> CV[CV: SimCLR/MoCo]
    CL --> NLP[NLP: SimCSE]
    CL --> Rec[推荐: CL4Rec/双塔]
    CL --> Search[搜索: DPR 对比训练]
```
...
$$
\frac{\partial \mathcal{L}}{\partial q} = \frac{1}{\tau}\left[\underbrace{(p^+ - 1) \cdot k^+}_{\text{正样本（拉近 q）}} + \underbrace{\sum_j p_j \cdot k_j^-}_{\text{负样本（推开 q）}}\right]
$$
