...
| 方案 | 之前方案 | 创新 | 核心突破 |
|------|---------|------|--------|
| TF-IDF | 布尔检索 | **词频-逆文档频率** | 可量化相关性 |
| BM25 | TF-IDF（无饱和） | **词频饱和 + 长度归一化** | 至今最强稀疏基线 |
| SPLADE | BM25（词级匹配） | **MLM 学习稀疏扩展** | 语义+稀疏结合 |
| 混合检索 | 单一稀疏或稠密 | **RRF 融合** | 互补优势 |

```mermaid
graph LR
    TF[TF-IDF] --> BM25[BM25]
    BM25 --> SPLADE[SPLADE 神经稀疏]
    SPLADE --> Hybrid[混合检索 RRF]
    Dense[Dense Retrieval] --> Hybrid
```
...
**记忆法**：BM25 就像"图书馆管理员打分"——IDF 是"这本书多稀有"（稀有词更重要），TF 饱和是"提到10次和100次差不多"（边际递减），b 参数是"长书不一定更相关"（长度惩罚）。k1 控制"多提几次还有用吗"，b 控制"长文档扣多少分"。

**核心考点**：
1. BM25 公式中 k1 和 b 分别控制什么？默认值多少？（k1=1.2 词频饱和，b=0.75 长度归一化）
2. BM25 相比 TF-IDF 的两大改进？（TF 饱和防止超线性增长 + 文档长度归一化）
3. 倒排索引的数据结构和查询时间复杂度？（词→文档列表映射，O(sum of df)）
4. RRF 融合为什么比分数加权更鲁棒？（只用排名信息，对量级差异和 outlier 免疫）
5. BM25 最大缺陷是什么？有哪些解决方案？（词汇不匹配，用 SPLADE/混合检索/查询扩展弥补）

**代码片段**：
```python
import numpy as np

def bm25_score(query_terms, doc_tf, doc_len, avg_dl, df, N, k1=1.5, b=0.75):
    """计算单个文档的 BM25 分数"""
    score = 0.0
...
# 示例
score = bm25_score(["广告", "点击"], {"广告": 3, "点击": 1}, doc_len=50,
                   avg_dl=100, df={"广告": 500, "点击": 2000}, N=10000)
print(f"BM25 score: {score:.4f}")
```

**常见踩坑**：
1. 混淆 BM25 的 IDF 公式和标准 IDF——BM25 用 log((N-df+0.5)/(df+0.5))，不是 log(N/df)
2. 忘记 b=0 时退化为无长度归一化、b=1 时完全按词密度——这是面试常问的极端情况
3. 认为 SPLADE 完全替代 BM25——实际工业中 BM25 仍是精确匹配不可或缺的基线
