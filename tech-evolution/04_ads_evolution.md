...
广告主设置目标 CPA（Target Cost Per Action）：target_CPA = 100 元

系统需要计算出价，使得期望 CPA = target_CPA

在单次竞价中：
  期望支付 ≈ bid（简化假设；严格言是 bid × P(赢)）
  期望转化数 ∝ pCTR × pCVR（用户点击后转化的概率）
  
  期望 CPA = 期望支付 / 期望转化
           = bid / (pCTR × pCVR)

令期望 CPA = target_CPA：
  bid / (pCTR × pCVR) = target_CPA
  
  ∴ bid = pCTR × pCVR × target_CPA

eCPM 计算（排序依据）：
  eCPM = bid × pCTR = pCTR × pCVR × target_CPA × pCTR
       = pCTR² × pCVR × target_CPA  (排序用)

实际校准系数 k（可选）：
  eCPM = k × pCTR × pCVR × target_CPA
  k 根据实际 CPA 与目标的差异自动调整（在线学习）
  k < 1 时保守出价，k > 1 时激进出价
```
...
#### 4.4 Bid Landscape 与赢率预测

**问题背景：** 对偶优化给出了 $$b_i^* = pCVR_i / \lambda^*$$，但 λ* 依赖于对手出价分布的准确估计。如果对手出价分布偏移，最优出价也会变化。

**赢率模型（Win Rate Prediction）：**

```
赢率定义：P(win | bid=b) = P(b > max_competitor_bid)

假设对手出价服从对数正态分布：
log(opponent_bid) ~ N(μ, σ²)

赢率计算（CDF）：
P(win | bid=b) = Φ((log(b) - μ) / σ)
...
- 每隔1小时更新一次 (μ, σ) 参数
- 按 {竞价场景、时段、用户类别} 分层估计
- 场景变化快时用在线学习（online convex optimization）
```
...
```
1. 预测赢率分布：
   P(win | bid) = F_opponent(bid)

2. 预测转化率：
   pCVR = 神经网络预测(用户、广告、上下文)

3. 对偶变量更新（每小时）：
   λ_t = λ_{t-1} - step_size × (Σ cost_t - Budget_expect_t)
...
**赢率模型常见错误：**
- ❌ 假设对手出价分布不变（实际出价会随流量/季节变化）
- ❌ 忽视自己出价对赢率的反馈效应（如果只出高价，看不到低价竞争）
- ✅ 定期重估 (μ, σ)，考虑时间衰减
...
**MPC 核心思想：** 离线规划一整天的出价轨迹，使得在已知的预测模型下，预算平滑消耗且最大化目标。

**与 PID 的区别：**

```
PID 反馈控制：
  状态 → 计算偏差 → 调整动作 → 观察新状态（当前时刻反应）
  λ(t) = K_p × e(t) + K_i × Σe(τ) + K_d × Δe(t)

MPC 预测控制：
  状态 → 未来 H 步轨迹预测 → 优化整个轨迹 → 执行第1步（提前规划）
  min_{λ_1,...,λ_H} L(states, actions) s.t. constraints
```
...
```
状态向量 x_t ∈ R^4：
  x_t = [预算消耗进度，转化数，时刻，流量质量]

预测模型（学习自历史）：
  x_{t+1} = f(x_t, λ_t) + 随机噪声

约束：
  - 预算：Σ_t cost(λ_t) ≤ Budget
  - 出价：λ_t ∈ [0.5, 2.0]  （相对于基础出价）

目标函数（H=12小时窗口）：
  J = Σ_{t=0}^{H-1} {转化数(x_t, λ_t) - 超预算惩罚(x_t, Budget)}
  
  ≈ max Σ_t pCVR_t × P(win | λ_t) - α × |Σ_t cost_t - Budget|²

最优轨迹求解：
  - 离线：每早上 6:00，用前一天数据训练预测模型 f(·)
  - 离线规划：求解 MPC，得到 λ*(1), λ*(2), ..., λ*(12)
  - 在线执行：按轨迹执行，每小时重计划（Receding Horizon）

重计划（Receding Horizon）机制：
  λ_today(1:12) = MPC.solve()  // 规划全天
  执行 λ_today(1) → 观察实际流量/转化
  λ_today(2:12) = MPC.replan()  // 重新规划剩余 11 小时（修正偏差）
```
...
```
✅ 优势：
  - 全局最优：考虑整个时间窗口，不是贪心局部最优
  - 约束自然满足：显式编码预算/时间约束
  - 可解释：规划的轨迹清晰可视化（下午 2 点降价，晚上 8 点提价）

❌ 局限：
  - 模型错误敏感：如果预测模型 f(·) 偏离实际，轨迹优化无效
  - 对手模型缺失：无法考虑竞争对手的出价变化（非合作博弈）
  - 不适应突变：突然流量激增、促销事件无法快速响应
  - 计算复杂度：非凸优化问题，求解耗时

典型 MPC 推理时间：2-5 分钟离线计算 → 12小时轨迹
```

**MPC 与 RL 的关系：**
- **MPC**：利用已知模型离线优化（模型驱动）
- **RL**：学习决策策略，在线适应（数据驱动）
- **混合**（最新趋势）：MPC 初始化 RL 的策略网络，RL 在线微调 MPC 轨迹
...
```
阿里妈妈 RL Bidding（双层架构）：
  上层：RL 智能体（DDPG）学习预算分配策略（小时级）
        - 状态：时刻、已消耗预算、已获转化、竞争强度
        - 动作：小时级 λ 系数
        - 算法：Deep Deterministic Policy Gradient (DDPG)
        
  下层：LP 对偶优化出价（秒级，实时响应竞价）
        - 接收上层小时 λ → 微调为秒级出价
        - 还是用 b = pCVR / λ，保证预算约束

字节跳动 Auto Bidding：
  使用 PPO 算法（Policy Gradient），更稳定
  状态包含竞争者出价分布估计（通过赢率反馈学习）
  离线仿真环境（Bidding Simulator）验证策略

美团 Auto Bidding：
  多目标 RL：同时优化 CVR、GMV、ROI
  考虑多广告主竞争（Multi-Agent 视角，部分竞争者的行为编入状态）
```

**在线仿真（Offline Training → Online Deployment）：**
```
1. 离线数据构建 Simulator：
   - 收集 1 个月竞价日志（Bid Request, 赢/输、支付价格、转化）
   - 训练 CTR/CVR 预测器、赢率预测器
   - 构建可重放的 Bidding Simulator（确定性）

2. RL 在 Simulator 中离线训练（避免线上 Exploration 风险）：
   - 初始随机策略，探索各种出价（0.5~2.0）
   - 在仿真环境中累积 10M 步经验
   - 策略网络逐步收敛（目标：最大化 CVR × (1 - cost/target_CPA)）

3. 线上金丝雀（Canary Deployment）：
   - 5% 流量使用新 RL 策略
   - 监控 KPI：CPA、转化数、ROI（vs 基线 PID）
   - 若 CPA ≤ 目标 ±5%，ROI ≥ 基线，则提升比例
...
**常见的 RL 坑：**
- ❌ 直接用线上流量训练（Exploration 可能降低 ROI）
- ❌ 不做 off-policy 纠正（历史数据的 bias）
- ❌ 奖励函数设计不当（e.g., 只优化转化数，导致烧钱）
- ✅ 始终在仿真环境离线训练，线上只做 Deployment
```
