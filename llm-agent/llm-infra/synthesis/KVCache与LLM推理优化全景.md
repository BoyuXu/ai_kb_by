...
> - [LMCache](https://arxiv.org/abs/2510.09665) — LMCache: An Efficient KV Cache Layer for Enterprise-Scale LLM Inference (2025)
> - [Online Scheduling for LLM Inference](https://arxiv.org/abs/2502.07115) — Online Scheduling for LLM Inference with KV Cache Constraints (MIT, 2025)

> 知识卡片 | 创建：2026-03-23 | 更新：2026-04-20 | 领域：llm-infra
...
$$
\text{KV}_{MHA} = 2 \times L \times H \times d_k \times N
$$
...
$$
\text{KV}_{GQA} = 2 \times L \times G \times d_k \times N = \frac{G}{H} \times \text{KV}_{MHA}
$$

**Multi-Query Attention（MQA，$G=1$）：**

$$
\text{KV}_{MQA} = 2 \times L \times 1 \times d_k \times N = \frac{1}{H} \times \text{KV}_{MHA}
$$
...
$$
\text{KV}_{GQA} = 2 \times 32 \times 8 \times 128 \times 8192 \times 2\text{ B} \approx 1.07\text{ GB}
$$
...
$$
\text{KV}_{Mem} = 2 \times L \times H_{\text{KV}} \times d_k \times N \times s
$$
...
$$
\text{KV}_{Mem} = 2 \times 80 \times 8 \times 128 \times 32768 \times 2 \approx 107\text{ GB}
$$
...
| 多租户 LLM 平台 | LMCache + PagedAttention | 共享 prefix，跨引擎 KV 复用，吞吐 15x |
| 高并发长上下文 | 在线调度 + KV 约束 | 防止 KV 溢出导致级联故障 |
...
XQuant KV 重物化 (2025) → 不存 KV 存 X，重算代替缓存，10-12.5x 压缩
    ↓
LMCache (2025) → GPU 外 KV Cache 层，跨引擎/跨请求共享，吞吐量 15x
    ↓
在线调度 + KV 约束 (2025) → 多项式时间调度算法，常数竞争比
    ↓（预测）
...
## 新增：LMCache — 企业级 KV Cache 共享层（2025）

> 论文：[LMCache: An Efficient KV Cache Layer for Enterprise-Scale LLM Inference](https://arxiv.org/abs/2510.09665)

**问题定义**：企业场景下，多用户/多引擎产生的 KV Cache 远超单 GPU 显存容量，无法跨请求/跨引擎复用，导致大量重复 prefill 计算。

**核心方法**：在 GPU 显存之外构建一个独立的 KV Cache 管理层，支持跨引擎（vLLM/SGLang）、跨请求的 KV Cache 提取、存储和共享。

**三大技术支柱**：

1. **高效数据搬运**：批量 KV 数据移动 + 计算与 I/O 流水线化，最小化 GPU↔CPU↔Storage 的传输延迟
2. **模块化 Connector**：解耦 LMCache 与推理引擎的演进，支持 vLLM 和 SGLang 双引擎
3. **一等公民控制 API**：灵活编排 KV Cache 在 GPU/CPU/磁盘/网络各层的分布
...
| 本地 | CPU 内存、本地磁盘 |
| 分布式存储 | NFS、WEKA、S3 |
| 高性能 | GPU-Direct Storage、Mooncake Store、NIXL |
| KV 数据库 | Redis、Valkey、InfiniStore |

**处理器支持**：NVIDIA、AMD、Ascend（华为）、TPU

**实验结果**：
- 结合 vLLM 使用，多轮 QA 和文档分析场景吞吐量提升 **15x**
- 远程 KV Cache 获取可显著降低 prefill 延迟
- 上下文截断会将 prefix cache 命中率降低约 50%（重要的工程教训）

**工业意义**：
- 第一个开源的企业级 KV Cache 管理系统
- 解决了 "相同 system prompt 每次重算" 的行业痛点
- 特别适合多租户 LLM 服务（同一模型、不同用户、大量共享前缀）

**面试考点**：LMCache 和 vLLM PagedAttention 的区别？答：PagedAttention 解决的是**单 GPU 内**的 KV Cache 碎片问题（内存分配效率）；LMCache 解决的是**跨 GPU/跨引擎/跨请求**的 KV Cache 共享和持久化问题（系统级缓存层）。两者互补：PagedAttention 管 GPU 内分配，LMCache 管 GPU 外流转。

---

## 新增：KV Cache 约束下的在线调度（MIT, 2025）

> 论文：[Online Scheduling for LLM Inference with KV Cache Constraints](https://arxiv.org/abs/2502.07115)

**问题定义**：LLM 推理服务中，KV Cache 是动态增长的——每个请求在 decode 过程中 KV Cache 线性增长，总 GPU 显存有限。如何在 KV Cache 内存约束下，最优地调度请求批次以最小化总延迟？

**核心方法**：提出一个多项式时间在线调度算法，在 KV Cache 容量约束下进行请求批处理决策。

**关键贡献**：

1. **形式化建模**：将 LLM 推理调度建模为带内存约束的在线调度问题
   - 每个请求有 prefill 阶段（计算密集）和 decode 阶段（KV Cache 增长）
   - KV Cache 占用 = f(已生成 token 数)，总容量受 GPU 显存限制
   - 目标：最小化所有请求的总完成时间

2. **常数竞争比保证**：在特定条件下，算法达到常数竞争比（相对于离线最优解的理论上界有界）

3. **实际验证**：在 Llama2-70B + A100 GPU 上的真实推理数据集上，显著优于 benchmark 算法

**核心调度思想**：

```
传统调度：贪心塞满 batch → KV Cache 溢出 → 驱逐/OOM → 性能崩溃
本文方法：预测 KV Cache 增长轨迹 → 控制 batch admission → 保证无溢出
```
...
| 方法 | 调度策略 | 内存保证 | 理论分析 |
|------|---------|---------|---------|
| vLLM 默认 | FCFS + preemption | 溢出时 swap/recompute | 无 |
| Orca | Continuous Batching | 无显式保证 | 无 |
| WAIT (Fluid-Based) | 阈值批处理 | 流体近似保证 | 近似最优 |
| **本文** | **在线算法** | **硬性 KV 约束** | **常数竞争比** |

**面试考点**：为什么 LLM 推理调度比传统 DNN 调度更难？答：(1) KV Cache 随 decode 动态增长（传统 DNN 内存固定）；(2) 输出长度未知（不知道何时结束）；(3) prefill 和 decode 计算特性不同（分别是计算密集和内存密集）；(4) 多请求共享 GPU 显存，互相竞争 KV Cache 空间。
...
- **KV Cache = 笔记本**：对话时记住之前说过的话（Key-Value），下次回答不用重头想，但笔记本会越来越厚（内存增长）
- **PagedAttention = 活页笔记本**：传统笔记本预留固定页数（浪费），活页本按需加页（vLLM 分页），不浪费纸张
- **GQA = 共享笔记**：多个 Attention Head 共享同一份 K/V 笔记（而非每人一份），省内存但信息损失极小
- **KV Cache 量化 = 笔记缩写**：把详细笔记压缩为缩写版本（2-4bit），省空间但有少量信息丢失
- **Continuous Batching = 餐厅翻台**：不等所有人吃完（static batching），有人吃完就上下一位（动态调度），提高翻台率

### 对比表

| 技术 | 优化对象 | 内存节省 | 精度影响 | 工程复杂度 |
|------|---------|---------|---------|-----------|
| 朴素KV Cache | 基线 | 0 | 0 | 低 |
| PagedAttention | 碎片消除 | ~60-80% | 0 | 中 |
| GQA/MQA | KV Head数 | 4-32× | <0.5% | 低（模型设计） |
| KV量化(2-4bit) | 精度压缩 | 4-8× | 1-3% | 中 |
| H2O/SnapKV | 重要性筛选 | 2-5× | <1% | 中 |

### 口诀/助记

- **KV Cache 三板斧**："分页管碎片（PagedAttention），共享省头数（GQA），量化压位宽"
- **推理两阶段**："Prefill 算力密集（大矩阵乘），Decode 内存密集（KV Cache 读写）"
- **vLLM 核心**："PagedAttention = OS 虚拟内存分页思想应用到 KV Cache"

### 面试一句话

- **PagedAttention**："借鉴 OS 虚拟内存分页，按需分配 KV Cache Block，消除预分配导致的 60-80% 内存浪费，并发能力从 batch=1-2 提升到 batch=16-64"
- **GQA**："多个 Query Head 共享同一组 K/V Head，LLaMA-3 用 8 组 GQA 将 KV Cache 压缩 4 倍，精度损失 <0.5%，是 2024+ 大模型标配"
