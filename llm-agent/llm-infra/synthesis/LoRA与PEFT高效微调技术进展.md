...
$$
h = W_0 x + \underbrace{\frac{\alpha}{r}_{缩放}} B A x
$$
...
- **LoRA = 贴便签**：不改原书（冻结参数），在关键页面贴便签（低秩矩阵 B×A），微调只训练便签，原书不动
- **QLoRA = 压缩原书+贴便签**：先把原书压缩成口袋版（4bit量化），再贴便签（LoRA），省了 75% 书架空间
- **Adapter = 加书签层**：在每一章之间插入薄薄的书签层（瓶颈网络），只训练书签，原章节不动
- **Prefix Tuning = 写序言**：在输入前面加一段可训练的"序言"（prefix token），引导模型按特定方式回答
- **BOFT = 旋转不变形**：用正交矩阵微调，保证微调后参数不失去原有结构（蝶形正交变换）

### 对比表

| 方法 | 可训参数占比 | 显存节省 | 效果 | 工程复杂度 |
|------|-----------|---------|------|-----------|
| 全参数微调 | 100% | 0 | 最好 | 低 |
| LoRA | <1% | ~50% | 接近全参数 | 低 |
| QLoRA | <1% | ~75% | 略低于LoRA | 中 |
| Adapter | <1% | ~50% | 接近全参数 | 低 |
| Prefix Tuning | 0.1% | ~60% | 略低 | 低 |

### 口诀/助��

- **LoRA 公式记**："W = W0 + (α/r)×B×A，W0 冻结，只训 B 和 A"
- **LoRA 秩选择**："r=4 够用，r=16 更好，r=64 基本等于全参数"
- **QLoRA 三板斧**："NF4 量化 + 双重量化 + 分页优化器"
- **PEFT 选型记**："资源充足用 LoRA，极度省钱用 QLoRA，多任务用 AdapterFusion"

### 面试一句话

- **LoRA 原理**："在冻结的预训练权重旁加一个低秩分解 ΔW=BA（r<<d），只训练 B 和 A，参数量减少 99%，效果接近全参数微调；推理时可合并回原权重，不增加延迟"
- **QLoRA**："先将基座模型量化到 4bit（NF4 格式），再在量化后的模型上训练 LoRA，显存节省 75%，使得在单张消费级 GPU 上微调 65B 模型成为可能"
