...
$$
\text{Conf}(a | q, \mathcal{D}) = \frac{1}{K}\sum_{k=1}^{K} \mathbb{1}[\text{LLM}(q, d_k) = a]
$$
...
**直观理解**：让 LLM 分别基于不同检索文档回答同一问题，答案一致性越高说明越可信。如果 5 个文档给出 4 个相同答案，置信度 0.8——这比只用一个文档生成答案要可靠得多。

### 公式 2：Agent ReAct 框架的决策

$$
a_t = \pi(o_t, m_{1:t-1}), \quad o_t = \text{Tool}(a_{t-1})
$$
...
**直观理解**：ReAct 让 LLM 交替"思考"（Reason）和"行动"（Act）。每次行动后观察结果，根据观察调整下一步。就像人解决问题：想一想→试一试→看结果→再想想→再试。比一步到位更鲁棒。

### 公式 3：Self-RAG 的自我反思打分

$$
\text{score}(y) = w_1 \cdot \text{IsRel}(d, q) + w_2 \cdot \text{IsSup}(y, d) + w_3 \cdot \text{IsUseful}(y, q)
$$
