...
**A:** RRF 公式 $\text{RRF}(d) = \sum_{q'} \frac{1}{k + \text{rank_{q'}(d)}$ 中的 $k$ 控制了排名差异的权重衰减速度。$k$ 越大，不同排名位置的分数差距越小（趋向均匀融合）；$k$ 越小，高排名文档获得更大优势。TongsearchQR 使用 $k=60$（经典值），但最优值取决于各路检索器的排名分布。工程建议：对质量相近的检索器用 $k=60$；如果某路检索器显著优于其他（如 Dense >> BM25），应降低 $k$ 以增大高质量排名的权重，或引入加权 RRF（对不同查询路的求和加权）。
