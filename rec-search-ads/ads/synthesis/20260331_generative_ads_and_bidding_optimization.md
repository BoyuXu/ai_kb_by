...
$$
r(\text{ad}) = w_1 \cdot \text{relevance}(\text{ad}, \text{query}) + w_2 \cdot \text{quality}(\text{ad}) + w_3 \cdot \text{revenue}(\text{ad})
$$

**直观理解**：LLM 生成广告文案时需要同时满足三个目标：和用户搜索意图相关、文案质量高、能带来收入。reward model 用加权方式融合，通过 RLHF 让 LLM 学会在三者间平衡。

### 自动出价的在线约束优化

$$
\min_{b(\cdot)} \; -\mathbb{E}[V \cdot \mathbb{1}[b(x) > p]] \quad \text{s.t.} \quad \mathbb{E}[p \cdot \mathbb{1}[b(x) > p]] \leq B
$$
...
**直观理解**：在预算有限的情况下，如何智能出价让总价值最大？答案是出价正比于 $V$（高价值用户出高价）但要受预算约束——不能前半天把预算花完后半天没钱投。

### 生成式创意的 A/B 检验

$$
\text{Z} = \frac{\text{CTR}_{\text{gen}} - \text{CTR}_{\text{manual}}}{\sqrt{\frac{\hat{p}(1-\hat{p})}{n_1} + \frac{\hat{p}(1-\hat{p})}{n_2}}}
$$

**直观理解**：LLM 生成的广告文案真的比人写的好吗？Z 检验量化两组 CTR 差异的统计显著性——$|Z| > 1.96$ 说明差异在 95% 置信度下不是偶然的。工业中通常要求连续 3 天显著才上线。
