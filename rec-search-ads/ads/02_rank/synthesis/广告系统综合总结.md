...
$$
\text{ROI} = \frac{\text{Revenue} - \text{Cost}}{\text{Cost}} = \frac{\sum_i \text{bid}_i \times \mathbb{1}[\text{win}_i] \times \text{CVR}_i \times \text{value}_i - \sum_i \text{cost}_i}{\sum_i \text{cost}_i}
$$
...
$$
\text{Fill\_Rate} = \frac{\text{有广告填充的请求数}}{\text{总请求数}}, \quad \text{eCPM\_floor}: \text{score} > \theta
$$
