...
$$
P(Y=y) = \begin{cases} \pi(x) & y = 0 \\ (1-\pi(x)) \cdot \frac{1}{y\sigma\sqrt{2\pi}} e^{-\frac{(\ln y - \mu(x))^2}{2\sigma^2}} & y > 0 \end{cases}
$$

- $\pi(x)$：零膨胀概率（用户不付费的概率）
- $\mu(x), \sigma$：LogNormal 的参数

**直观理解**：大部分用户 LTV 为零（不付费），少数付费用户的金额服从长尾分布。ZILN 先判断"会不会付费"（$\pi$），再对付费用户预测"付多少"（LogNormal）。比直接回归更准——因为"零"和"非零"是两种完全不同的行为模式。
...
**直观理解**：明天的一块钱不如今天的一块钱值钱（时间价值）。LTV 把未来每期的净收益折现到现在——一个"一年后付 1000 元"的用户 LTV 低于"今天就付 900 元"的用户。折现率反映资金的机会成本。

### 分位数回归损失

$$
\mathcal{L}_\tau(y, \hat{y}) = \tau \cdot \max(y - \hat{y}, 0) + (1-\tau) \cdot \max(\hat{y} - y, 0)
$$
