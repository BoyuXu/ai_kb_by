...
$$
h_k = \text{RNN}(h_{k-1}, x, \text{time\_features}_k), \quad h_k(x) = \sigma(W \cdot h_k + b)
$$
...
$$
\text{CVR}_{final} = \text{CVR}_{model} \times \text{correction\_factor}(\text{elapsed\_time})
$$
...
$$
\text{bid} = \text{target\_CPA} \times \text{pCVR} \times k
$$
...
$$
\text{bid}_{biased} = \text{target\_CPA} \times \underbrace{\text{pCVR}_{obs}}_{\text{低估}} \times k < \text{bid}_{true}
$$
...
$$
\text{bid}_{corrected} = \text{target\_CPA} \times \text{pCVR}_{obs} \times \text{CF}(e) \times k
$$
