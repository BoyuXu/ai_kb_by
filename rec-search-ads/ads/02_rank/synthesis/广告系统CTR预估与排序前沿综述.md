...
$$\text{MaskBlock}(X) = \text{FFN}\left(\text{LayerNorm}\left(\text{MLP}_{mask}(X) \odot X\right)\right)$$
...
- $\text{MLP}_{mask}(X)$：基于输入实例动态生成的掩码
- **意义**：引入实例引导的乘性特征交互，突破加性 FFN 的局限

### 公式2: DHEN 层次集成

$$h_l = \text{Aggregate}\left(\{\text{Module}_k(h_{l-1})\}_{k=1}^K\right)$$
