...
> - [Bm25-Semantic-Hybrid-Retrieval](../papers/BM25_Hybrid_Retrieval.md) — BM25 与语义检索融合：Hybrid Retrieval 最佳实践
...
> - [Leser-Lexical-Semantic-Retrieval](../papers/LeSeR_Lexical_Semantic_Retrieval_for_Regulatory_Informati.md) — LeSeR: Lexical-Semantic Retrieval for Regulatory Informat...
...
$$
\text{score}}_{\text{{\text{fused}}} = \alpha \cdot \frac{s_{\text{dense}} - \min}{\max - \min} + (1-\alpha) \cdot \frac{s_{\text{sparse}} - \min'}{\max' - \min'}
$$

**直观理解**：不同检索器的分数范围差异巨大（BM25 可能 0-30，cosine 可能 0-1），直接加权没意义。先 min-max 归一化到 [0,1]，再加权融合。$\alpha$ 的最佳值通常在 0.5-0.7（稍偏向语义检索），需要在验证集上调。

### 多路召回去重与排序

$$
\text{final}}_{\text{{\text{list}}} = \text{Dedup}(\bigcup_c \text{TopK}}_{\text{c), \quad \text{ranked by Reranker}}
$$

**直观理解**：不同召回通道（BM25、向量、图谱、热门）的结果取并集去重后送入 reranker 精排。并集越大覆盖越全，但 reranker 处理更多候选就更慢——通常每路取 top-100，去重后 200-500 条送精排。
...
**直观理解**：如果两个检索器找到的相关文档高度重叠（互补性低），融合没意义（1+1≈1）。BM25 和 Dense 的互补性通常很高（擅长不同类型的 query），这就是混合检索有效的根本原因。
