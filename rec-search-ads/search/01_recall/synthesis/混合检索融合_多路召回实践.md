...
$$
\text{score}_{\text{fused}} = \alpha \cdot \frac{s_{\text{dense}} - \min}{\max - \min} + (1-\alpha) \cdot \frac{s_{\text{sparse}} - \min'}{\max' - \min'}
$$
...
$$
\text{final\_list} = \text{Dedup}(\bigcup_c \text{TopK}_c), \quad \text{ranked by Reranker}
$$
