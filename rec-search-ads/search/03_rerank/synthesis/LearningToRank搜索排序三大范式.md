...
$$
\lambda_{ij} = \frac{-1}{1 + e^{s_i - s_j}} \cdot |\Delta\text{NDCG}}_{\text{{ij}}|
$$
...
2. **LambdaRank 的关键改进**：将 RankNet 梯度乘以 $|\Delta\text{NDCG}}_{\text{{ij}}|$（交换文档 $i,j$ 的位置后 NDCG 的变化量）：
...
$$
|\Delta\text{NDCG}}_{\text{{ij}}| = \left|\frac{1}{\text{IDCG}}\left(\frac{2^{rel_i}-1}{\log_2(\text{rank}}_{\text{i+1)}} + \frac{2^{rel_j}-1}{\log_2(\text{rank}}_{\text{j+1)}} - \frac{2^{rel_i}-1}{\log_2(\text{rank}}_{\text{j+1)}} - \frac{2^{rel_j}-1}{\log_2(\text{rank}}_{\text{i+1)}}\right)\right|
$$
...
- $|\Delta\text{NDCG}}_{\text{{ij}}|$：交换 $i, j$ 位置后的 NDCG 变化量（衡量该对的重要性）
