...
$$
\lambda_{ij} = \frac{-1}{1 + e^{s_i - s_j}} \cdot |\Delta\text{NDCG_{ij}}|
$$
...
2. **LambdaRank 的关键改进**：将 RankNet 梯度乘以 $|\Delta\text{NDCG_{ij}}|$（交换文档 $i,j$ 的位置后 NDCG 的变化量）：
...
$$
|\Delta\text{NDCG_{ij}}| = \left|\frac{1}{\text{IDCG}}\left(\frac{2^{rel_i}-1}{\log_2(\text{rank}_{\text{i+1)}} + \frac{2^{rel_j}-1}{\log_2(\text{rank}_{\text{j+1)}} - \frac{2^{rel_i}-1}{\log_2(\text{rank}_{\text{j+1)}} - \frac{2^{rel_j}-1}{\log_2(\text{rank}_{\text{i+1)}}\right)\right|
$$
...
- $|\Delta\text{NDCG_{ij}}|$：交换 $i, j$ 位置后的 NDCG 变化量（衡量该对的重要性）
...
- **Pointwise = 单独评分**：像老师给每篇作文独立打分（回归/分类），不考虑其他作文
- **Pairwise = 两两PK**：像擂台赛，每次两篇作文比谁好（RankNet/LambdaRank），积累胜负决定排名
- **Listwise = 整体排序**：像选秀节目一次性排出所有选手名次（ListNet/LambdaMART），直接优化排序指标
- **NDCG = 带折扣的得分榜**：排名越靠前权重越大（折扣递减），高相关文档排前面奖励高
- **LambdaRank = 虚拟梯度**：NDCG 不可微，直接定义"如果交换两个文档位置会让 NDCG 变化多少"作为梯度
...
| Pairwise | RankNet/RankSVM | 成对偏序 | 部分（两两比较） | 中 |
| Listwise | LambdaMART/ListNet | 排序指标 | 是（全列表） | 高 |
| Neural | BERT Reranker | Cross-Attention | 是（查询-文档交互） | 高 |

### 口诀/助记

- **LTR三范式**："单独打分→两两PK→整体排名" — Pointwise → Pairwise → Listwise
- **NDCG 公式记忆**："分子是相关性指数（2^rel-1），分母是位置折扣（log2(i+1)），除以理想值归一"
- **LambdaMART = LambdaRank + GBDT**："虚拟梯度 + 梯度提升树 = 搜索排序 SOTA"

### 面试一句话

- **LTR 三范式**："Pointwise 独立评分忽略排序信息；Pairwise 学文档对偏序但无法直接优化 NDCG；Listwise 通过 LambdaRank 定义虚拟梯度直接优化 NDCG，LambdaMART 是工业 SOTA"
- **NDCG 不可微问题**："NDCG 涉及排序操作不可微，LambdaRank 的巧妙之处在于直接定义梯度 λ_ij = σ'(s_i-s_j) × |ΔNDCG|，绕过了对损失函数求导"
