...
$$
\text{bid}_{	ext{t+1}} = \text{bid}_{t} + K}_{\text{p \cdot e}_{t} + K}_{\text{i \sum}_{	ext{j=0}}^{t} e_j + K_d (e_t - e_{t-1})
$$
...
$$
\text{出价}_{	ext{oCPC}} = \text{CPA}_{	ext{target}} \times p(\text{convert} | \text{click})
$$
