...
审视概率随位置衰减，常用幂律建模：$P(\text{exam}|\text{pos}_k) = f(k)$，典型形式为 $f(k) = k^{-\eta}$，其中 $\eta \approx 0.5 \sim 1.0$。

### 5. IPW 损失函数

$$
\mathcal{L}_{IPW} = \sum_{i} \frac{\ell(y_i, \hat{y}_i)}{P(\text{exam}|\text{pos}_i)}
$$
...
$$
\hat{R}_{SNIPS} = \frac{\sum_i w_i r_i}{\sum_i w_i}, \quad w_i = \frac{1}{P(\text{exam}|\text{pos}_i)}
$$
...
$$
\text{DI} = \frac{P(\text{exposure}|\text{group}_a)}{P(\text{exposure}|\text{group}_b)}
$$
...
$$
\mathcal{L}_{debias} = \sum_i \ell(y_i, \hat{y}_i - b_{\text{pos}_i})
$$

- 训练时学习位置偏置 $b_{\text{pos}_i}$，推理时不加入
...
$$
\text{Bias}_{UT} = E_{\pi_0}[r] - E_{\pi_{\text{uniform}}}[r]
$$
