...
> - [Counterfactual-Learning-For-Unbiased-Ad-Ranking...](../../ads/02_rank/papers/Counterfactual_Learning_for_Unbiased_Ad_Ranking_in_Indust.md) — Counterfactual Learning for Unbiased Ad Ranking in Indust...
...
> - [Jensen-Gap-Fairness-Rec](../../rec-sys/04_multi-task/papers/Bridging_Jensen_Gap_for_Max_Min_Group_Fairness_Optimizati.md) — Bridging Jensen Gap for Max-Min Group Fairness Optimizati...
> - [Query As Anchor Llm User Repr](../../search/01_recall/papers/Query_as_Anchor_Scenario_Adaptive_User_Representation_via_v2.md) — Query as Anchor: Scenario-Adaptive User Representation vi...
> - [Query-As-Anchor-Scenario-Adaptive-User-Represen...](../../search/01_recall/papers/Query_as_Anchor_Scenario_Adaptive_User_Representation_via.md) — Query as Anchor: Scenario-Adaptive User Representation vi...
...
$$
\text{Overall}_{CVR} = \prod_{k=1}^{K} P(\text{stage_{k | }\text{stage}_{k-1})
$$
...
$$
\text{score}_{final}(i) = \max_c \text{score_{c(i), }\quad \text{or}} \quad \text{score}_{final}(i) = \sum_c w_c \cdot \text{norm}(\text{score}_{\text{c(i))
$$
...
$$
T}_{total} \leq T_{\text{recall}} + T_{\text{prerank}} + T_{\text{rank}} + T_{\text{rerank}} + T_{\text{network}}
$$
...
- **推荐系统漏斗 = 选秀节目**：海选（召回，亿→千）→ 初赛（粗排，千→百）→ 复赛（精排，百→十）→ 决赛（重排，十→展示）
- **召回 = 撒网捕鱼**：多路撒网（协同/向量/图/热门），宁可多捞不漏掉好鱼
- **精排 = 面试官**：每个候选仔细评估，DIN/DCN 等复杂模型打分
- **重排 = 导演排节目单**：不只看单个节目好不好，还要整体搭配多样化，避免连续三个相声
- **A/B 测试 = 药物临床试验**：实验组和对照组对比，统计显著才上线

### 口诀/助记

- **全链路口诀**："召排粗精重，层层漏斗筛"
- **面试设计题六步法**："场景指标→召回策略→排序模型→重排多样→在线实验→工程架构"
- **每层数量级**："亿→千→百→十→展示"
- **关键指标记**："召回看 Recall@K，排序看 AUC，线上看 CTR/GMV"

### 面试一句话

- **全链路串联**："推荐系统是从亿级候选到几十条展示的多阶段漏斗，每层权衡效率与精度：召回求覆盖（多路并行），精排求精准（DIN+多任务），重排求体验（多样性+业务规则）"
- **各层延迟**："召回 <5ms，粗排 <10ms，精排 <50ms，重排 <20ms，总计 <100ms"
