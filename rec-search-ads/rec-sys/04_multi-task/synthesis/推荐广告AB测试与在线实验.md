...
**直观理解**：实验组 CTR 比对照组高了 0.5%——这是真的提升还是随机波动？Z 值越大说明差异越不可能是偶然的。$|Z| > 1.96$ 对应 95% 置信度，$|Z| > 2.58$ 对应 99% 置信度。
...
**直观理解**：给定样本量 $n$，能检测到的最小效果是多少？如果想检测 0.1% 的 CTR 提升，可能需要数千万样本。这决定了"实验要跑多久"——流量小的平台需要跑更久，流量大的平台几天就够。

### 网络效应下的分桶

$$
\text{SUTVA violation}: Y_i(T) \neq Y_i(T'), \quad \text{when neighbors of } i \text{ differ in } T \text{ vs } T'
$$
