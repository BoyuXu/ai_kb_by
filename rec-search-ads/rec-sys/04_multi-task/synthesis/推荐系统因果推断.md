...
$$
\hat{\mathcal{L}_{IPS}} = \frac{1}{|\mathcal{O}|} \sum_{(u,i) \in \mathcal{O}} \frac{e_{u,i}}{P(O_{u,i} = 1 \mid \mathbf{x}_{u,i})}
$$
...
$$
\mathbb{E}[\hat{\mathcal{L}_{IPS}}] = \mathbb{E}\!\left[\sum_{u,i} O_{u,i} \cdot \frac{e_{u,i}}{P(O_{u,i}=1)}\right] = \sum_{u,i} \underbrace{P(O_{u,i}=1)}_{\mathbb{E}[O_{u,i}]} \cdot \frac{e_{u,i}}{P(O_{u,i}=1)} = \sum_{u,i} e_{u,i}
$$
...
$$
\hat{\mathcal{L}_{DR}} = \frac{1}{|U||I|}\sum_{u,i} \hat{e}_{u,i} + \frac{1}{|\mathcal{O}|}\sum_{(u,i) \in \mathcal{O}} \frac{e_{u,i} - \hat{e}_{u,i}}{P(O_{u,i}=1)}
$$
...
分解"点击"为"相关且被检查"，通过 Position Propensity 模型估计 $P(\text{examine} \mid \text{pos})$，然后用 IPS 校正训练信号：$w_{u,i} = 1 / P(\text{examine} \mid \text{pos_{u,i}})$。
