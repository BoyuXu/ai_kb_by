...
$$
\mathcal{L}_{\text{align}} = \text{InfoNCE}(\text{Proj}_{\text{rec}}(e_{\text{item}}), \text{Proj}_{\text{llm}}(e_{\text{text}}))
$$

**直观理解**：推荐模型和 LLM 的 embedding 空间完全不同——推荐 embedding 是从行为数据学的，LLM embedding 是从文本学的。对齐损失让同一个物品在两个空间中的投影相近，实现跨模态知识传递。

### Instruction Tuning for Rec

$$
\text{response} = \text{LLM}(\text{"根据用户历史 [行为列表]，在 [候选物品] 中选择最可能点击的 3 个"})
$$

**直观理解**：通过指令微调让通用 LLM 学会"做推荐"。关键在于指令格式——把协同过滤信号翻译成自然语言（"用户 A 和用户 B 都喜欢物品 X，用户 B 还喜欢物品 Y"），让 LLM 在语言空间中做推理。

### 增量推理

$$
\text{Cache}(u) \leftarrow \text{Cache}(u) \cup \{e_{\text{new\_action}}\}, \quad \hat{y} = f(\text{Cache}(u), e_{\text{candidate}})
$$

**直观理解**：每次用户产生新行为（点击/购买），不需要重新计算整个用户表示，只需把新行为的 embedding 加入缓存并增量更新。对于 Transformer 就是追加 KV Cache，对于 Mamba 就是更新隐状态——实时性的关键。
