...
$$
\hat{y}_{\text{cold}} = f(\text{content\_features}(i)), \quad \text{content} = [\text{title\_emb, category, price, ...}]
$$
...
$$
\text{traffic}(i) = \begin{cases} \text{exploit\_ratio} \times \text{score}(i) & \text{hot items} \\ \text{explore\_ratio} / |\text{cold\_items}| & \text{cold items} \end{cases}
$$
