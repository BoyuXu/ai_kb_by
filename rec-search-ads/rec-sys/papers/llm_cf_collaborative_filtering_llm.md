...
A：静态策略：交叉验证调参；动态策略：基于用户活跃度自适应调权（活跃用户CF数据丰富，权重高；冷启动用户语义权重高）。最佳实践是用可学习的注意力机制：$\alpha = \text{sigmoid}(\mathbf{w}^T [\mathbf{e}^{CF}, \mathbf{z}^{LLM}, \text{activity\_feat}])$，根据用户特征自动决定融合比例。
