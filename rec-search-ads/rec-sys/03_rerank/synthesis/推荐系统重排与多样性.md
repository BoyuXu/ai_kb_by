...
- **重排 = 花束搭配**：精排给了一堆最好的花（高分物品），但全是玫瑰太单调，重排要搭配不同品种（多样性），同时保证整束花好看（相关性）
- **MMR = 不要撞衫**：每次选一个"最好看但和已选的最不像"的，避免推荐列表全是同类商品
- **DPP = 体积最大化**：选的物品向量在空间中"撑开的体积"越大越好——向量越分散（多样）、越长（相关），体积越大
- **PRM = 班级座位安排**：不只看每个学生成绩（pointwise），还要考虑相邻座位的搭配效果（listwise），用 Transformer 建模"谁坐谁旁边效果最好"

### 对比表

| 方法 | 核心机制 | 计算复杂度 | 多样性质量 | 工业使用率 |
|------|---------|-----------|-----------|-----------|
| 规则去重 | 类目/品牌打散 | O(N) | 低 | 很高 |
| MMR | 贪心相关性-相似度 | O(N×K) | 中 | 很高 |
| DPP | 行列式最大化 | O(N³)/贪心O(N²K) | 高 | 低 |
| PRM | Transformer listwise | O(N²d) | 高 | 中 |
| RL重排 | 强化学习序列决策 | 高 | 高 | 低（前沿） |

### 口诀/助记

- **重排三要素**："多样性、新颖性、业务规则"
- **MMR 一句话**："λ 控平衡，1 全相关，0 全多样，0.5-0.7 刚好"
- **DPP 直觉记**："行列式 = 体积，向量分散体积大，分散就是多样"
- **信息茧房三指标**："覆盖率（推了多少比例的库）、基尼系数（分布均匀度）、ILS（列表内相似度）"

### 面试一句话

- **重排作用**："重排是推荐的最后一公里，在精排分数基础上注入列表级多样性（MMR/DPP）、新颖性、业务规则（品牌打散/广告插入），直接决定用户实际看到的结果"
- **MMR vs DPP**："MMR 贪心逐步选择，计算简单 O(NK)；DPP 通过行列式联合考虑全集合多样性，理论最优但 MAP 推断 NP-hard，贪心近似后效果和 MMR 差不多但实现更复杂"
