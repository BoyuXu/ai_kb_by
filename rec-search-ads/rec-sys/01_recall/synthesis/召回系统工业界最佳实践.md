...
- **YouTube DNN = 超大规模选秀**：百万视频是选手，用 softmax 多分类选出你最想看的，用户向量是"评委口味"
- **PinSage = 社交圈传播**：Random Walk 就像"朋友的朋友可能认识"，图上走几步就能发现隐藏的关联
- **TDM = 树形目录查找**：从根节点（"所有视频"）逐层细分（"电影→动作→漫威→钢铁侠"），O(logN) 就找到目标
- **多路分数校准 = 统一度量衡**：不同召回源的分数就像"公斤 vs 磅 vs 斤"，必须统一换算后才能比较
- **HNSW = 问路导航**：从"高速公路"（顶层稀疏连接）定位大致区域，再走"小路"（底层稠密连接）精确找到目的地

### 对比表

| 工业方案 | 核心优势 | 适用规模 | 落地难度 |
|---------|---------|---------|---------|
| YouTube DNN | 经典架构，工程成熟 | 百万级 | 低 |
| 快手负采样 | 样本工程精细 | 亿级 | 中 |
| PinSage | 图结构捕捉高阶相似 | 十亿级 | 高 |
| TDM | 突破内积限制 | 亿级 | 很高 |
| EGES | 冷启动友好 | 十亿级 | 中 |
| HNSW | 毫秒级ANN | 亿级 | 低 |
...
- **双塔训练三板斧**："正样本选点击，负样本看热度，纠偏减 log p"
- **负样本铁律**："曝光未点击不做召回负样本！它已经过了精排筛选"
- **多路融合四招**："归一化→渠道加权→模型融合→RRF保序"
- **HNSW三参数**："M管连接数，efConstruction管建图质量，efSearch管查询精度"

### 面试一句话

- **为什么是双塔不是交叉塔**："交叉塔需要对每个 (user, item) 对单独推理，亿级候选不可能；双塔 user 和 item 分别编码，item 离线算好建索引，在线只做 ANN 查找"
- **HNSW 原理**："多层图结构，上层稀疏长距离连接快速定位，下层稠密短距离连接精确搜索，查询复杂度 O(logN)，工业中 ef=200 可达 99%+ recall@100"
