...
**直观理解**：和 LLM 类似，推荐模型也存在 scaling law——模型越大、数据越多，loss 越低。Wukong 的发现是推荐系统的 $\alpha$ 比 LLM 小（模型增大收益递减更快），意味着盲目堆参数不如优化架构。

### Wukong 的分解式交叉

$$
\mathbf{y} = \sigma(\mathbf{W}_1 \mathbf{x}) \odot \sigma(\mathbf{W}_2 \mathbf{x})
$$

**直观理解**：把一个大矩阵乘分解为两个小矩阵乘的逐元素乘积，参数量减半但表达力不降。类似于 SwiGLU 的 gate 机制——一半做变换，一半做门控，门控决定哪些信息通过。

### 宽度 vs 深度的效率前沿

$$
\text{FLOPs} = L \times (d_{\text{model}}^2 + d_{\text{model}} \times d_{\text{ffn}})
$$
