...
**直观理解**：AUC = "随机抽一个正样本和一个负样本，模型给正样本的分数更高的概率"。AUC=0.75 意味着 75% 的情况下模型能正确排序。工业中 AUC 提升 0.1% 就可能带来显著的线上收益，因为每天有数十亿次排序。

### 特征重要性分析

$$
\text{FI}(f) = \text{AUC}_{\text{full}} - \text{AUC}_{\text{without\_f}}
$$
...
$$
\text{Priority}(\text{topic}) = \text{面试频率} \times \text{理解深度gap} \times \text{可迁移性}
$$

**直观理解**：学什么最值得？面试常考（频率高）、自己还不太懂（gap 大）、学了以后很多场景都能用（可迁移性强）的话题优先级最高。不要平均分配时间——二八定律，20% 的核心知识覆盖 80% 的面试题。
