...
| 方案 | 之前方案 | 创新 | 适用场景 |
|------|---------|------|---------|
| 线性加权 | 固定权重人工调参 | 简单直观 | 目标不冲突时 |
| MMoE/PLE | Hard-share（任务冲突时跷跷板） | **Expert+Gate 自适应共享** | 多任务学习 |
| Pareto 优化 | 线性加权（只能找凸面上的解） | **多目标梯度下降** | 目标严格冲突 |
| 约束优化 | 无约束多目标 | **拉格朗日乘子法** | 有硬约束（预算/ROI） |
...
```mermaid
graph LR
    LW[线性加权<br/>人工调权重] --> MMoE[MMoE/PLE<br/>Expert+Gate]
    MMoE --> Pareto[Pareto优化<br/>多目标梯度]
    Pareto --> Constrained[约束优化<br/>拉格朗日/KKT]
    Constrained --> AutoML[AutoML 搜索<br/>自动权重调优]
```

---

> 📚 参考文献与源文件
> - [[广告系统多目标优化|广告系统多目标优化]] — 广告收入 vs 用户体验的权衡
> - [[推荐系统排序范式演进|推荐系统排序范式演进]] — 多任务模型与目标平衡
> - [[推荐系统重排与多样性|推荐系统重排与多样性]] — 多目标约束优化
> - [[生成式排序范式|生成式排序范式]] — 端到端多目标优化
